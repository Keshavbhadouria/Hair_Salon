		...
		foreach($_POST["search"] as $k=>$v){
			if(!empty($v)) {
				...
				switch($k) {
					case "name":
						$name = $v;
						$queryCondition .= " sl_saloon_client.VchFirstName LIKE '" . $v . "%'";
						break;
					case "email":
						$email = $v;
						$queryCondition .= " sl_saloon_client.VchEmail LIKE '" . $v . "%'";
						break;
				}
			}
		}
	}
	$orderby = "ORDER BY intAppointmentId desc"; 
	$sql = "SELECT  wp_saloon_branch.*,tbl_services.*,sl_saloon_client.*,saloon_staff.*,wp_saloon_appointment.* from  wp_saloon_appointment
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      <tr>
		...
		<td><?php echo $result[$k]->VchFirstName;?></td>
		<td><?php echo $result[$k]->VchLastName;?></td>
		<td><?php echo $result[$k]->VchEmail;?></td>
		<td><?php echo $result[$k]->VchPhoneNo;?></td>
		<td><?php echo $result[$k]->vchAppointmentDate;?></td>
		<td><?php echo $result[$k]->vchFromTime.'- '.$result[$k]->vchToTime;?></td>
		<td><?php echo $result[$k]->vchSalon_Staff_FirstName;?></td>
		<td><?php echo $result[$k]->vchSaloonBranch;?></td>
		<td><?php echo $result[$k]->serviceName;?></td>
		
		<td><?php 
				$dellink  = admin_url('admin.php').'/wp-admin?page=solanbooking&action=delete&tid='.$result[$k]->intAppointmentId;
				?>
